...
### Story spine (NEW)
- **Four-fold seal canon** documented in `lore/world.md` + `lore/timeline.md`. Two centuries before the game opens, a Sealing Order led by **Yorath Tellan-Brenn** (Brenna's great-aunt) split a king-who-would-not-die into four parts: *the Crown* (body) → Hollow King, *the Pact* (oaths) → Drowned Warden, *the Memory* (name) → The Forgotten, *the Will* (what's left, sitting empty on the throne) → Crownless One. The bosses ARE those four parts; killing them dismantles the shape, not the threat.
- **Boss-codex auto-unlock** — `BOSS_CODEX` in `combatStore.ts`. Each main-boss kill drops a fragment into `loreStore`. Read in order they assemble the spine ("I broke the Crown..." → "The promises are loose..." → "The erasure is gone..." → "I do not know what I freed").
- **Brenna revelation cutscene** (`brenna-revelation.json`, 16 nodes) auto-fires when entering the Adventurers' Guild with 2+ main bosses dead and `hc_revelation_seen` unset (gating in `InteriorScene.ts:guildLayout`). Brenna names Yorath, explains the Sundering, and tells the player what they have actually been doing. Player branches: `hc_revelation_continue` / `hc_revelation_stop` / "I need to think." Future content can branch on these flags.
- **Aelia Ren journal arc** — 6 scattered pages across Mossbarrow Depths F1-3 + Drowned Sanctum F1 + Ashen Tower F1. The Sundering-era cartographer-sealer's voice; ties the four-fold seal to the player's lived experience without locking the spine.

### Companions (NEW)
- **3 new recruitable companions** following the Mira 3-beat-arc template (encounter → confession → recruit):
  - **Lyra Ashen** — Guild Deserter, Mossbarrow Cairn (south-west cairn stone). Rank D+ unlocks the truthful path; below D the "Brenna sent me" line is read as a bluff. Kit: `bonusDamage: 4`, `hireCost: 200`.
  - **Halvor Trent** — Sundering Scholar, Greenhollow clearing (forest stump). Beat-1 truthful path gates on `min-level: 5`. Relocates to the inn on agreement. Kit: `healPerTurn: 2`, `hireCost: 0`.
  - **Quill** — Former Hollow King Keeper, inn corner table (only spawns after `hc_hollow_king_defeated`). Kit: `bonusAc: 2`, `hireCost: 0`.
- 9 new dialogue files (`lyra-* / halvor-* / quill-*`). `DialogueScene.readFlag` now accepts both `'1'` and `'true'` so the legacy boss-defeat flag form (written as `'1'` by `combatStore`) routes through the existing `world-flag` predicate.

### NPC personal quests (NEW)
- **6 quests, one per anchor NPC**, each hitting 2+ DESIGN.md §9.5 emotional notes. All use existing items + monsters + scenes; no new world content.
  - **Brenna — *The Deserter's Letter*** (sad + hard decision): find Hesta hiding in Orric's cabin; lie (more XP) or tell the truth (more gold).
  - **Orric — *The Cloak in the Bramble*** (heartbreak + sad): wolves wearing the dead daughter's cloak. Sets `hc_orric_daughter_known` — opens a reactive "friend" line.
  - **Tomas — *The Empty Room*** (sad + puzzle + hard decision): recover a lost guest's journal from Mossbarrow; 3 recipient paths (Tomas keeps it / Tomas defers to Vira / direct to Vira).
  - **Vira — *The Last Ledger*** (hard decision + heartbreak): collect 40g from a debtor in Duskmere with a sick child, OR pay it yourself.
  - **Kael — *The Forge He Couldn't Light*** (heartbreak + puzzle): gather 2 iron + 1 bone + 1 moonpetal; lay the finished sword on Roa's grave (new examine interactable).
  - **Veyrin — *The Last Seal*** (hard decision + heartbreak + sad): reinforce his failing seal (gather 1 silver + 1 shadow + 1 wraith), OR end his suffering (sets `hc_seal_failed` + `hc_veyrin_dead` for future content to react to).
- 2 new NPCs (Hesta in Orric's cabin while quest active; Halden in south Duskmere). 6 new quest JSONs + 2 new dialogue files. Existing 5 greeting files extended (guild / orric / tomas / vira / veyrin).

### Environmental lore (NEW)
- **22 passive lore entries** scattered across world zones. Each persists "first read" via `worldStateStore.pickedObjects` so the message banner only fires once.
  - **12 gravestones**: 4 in Greenhollow, 3 in Mossbarrow, 3 in Mossbarrow Depths F1, 2 in Drowned Sanctum F1. Names + epitaphs in Souls register ("OWAIN. He laughed once. We held it. We hold it still.", "[name worn smooth] / who tried.", "TOMAS REN — Took the lantern down. Did not bring it up.").
  - **6 journal pages** by Aelia Ren — see Story spine.
  - **4 broken signs/inscriptions**: 2 Greenhollow waystones, 1 Mossbarrow arch, 1 Drowned Sanctum slab.
- Three new `BaseWorldScene` helpers: `spawnGravestone`, `spawnJournalPage`, `spawnBrokenSign`. All entries unlock via `loreStore.discover()` and group by `location` (which matches `getZoneName()`).
...
|---|---|
| _(audit + 8-agent fix fleet)_ | **Bug audit pass + parallel fixes.** New canonical artifact: `BUG_AUDIT.md` at repo root — 24 issues catalogued across HUD, world map, tile rendering, combat, save format, lifecycle, dev tooling. Eight agents launched in parallel (one cluster per file).<br>**HUD/pause** (`InGameOverlay.tsx`+`.css`): cheats panel + cheat-input JS eval gated to `import.meta.env.DEV` (zero hits in prod bundle); HP/MP/STA bar slots fixed-grid so layout doesn't reflow when class lacks MP/STA; smooth red→yellow→green HP color ramp replaces 50%/25% step; stamina recolored `#d4a03a` to distinguish from HP green; weather span suppressed when its icon duplicates the time-of-day icon (no more two ☀); body overflow lock on every modal so pause/options/etc don't trigger page scrollbars.<br>**Combat** (`CombatOverlay.tsx`): multi-enemy add HP bars now color-ramp like the primary. Audit was wrong on "Tab cycle hint vs unwired key" — verified Tab/Shift+Tab cycling is wired in `useEffect` + `cycleTargetIndex`.<br>**World Map** (`WorldMap.tsx`): added 9 missing zones (Ironveil Mines, 3 Bog Dungeon floors, Frosthollow Peaks, 3 Frozen Hollow floors, Forgotten Cave) with biomes/connections matching scene exits; relaxed the rigid Y=120 grid into a hub-and-branch layout (forest fork west, mountain branch NE, volcanic/coastal arc east, dungeons hang under their parent surface zone).<br>**Tile decoration** (`decorationOverlays.ts`): `cellHash` now mixes `scene.scene.key` via FNV-1a so tree variant + density cull differ per-zone; same `(x,y)` no longer renders identically across Greenhollow/Duskmere/Mossbarrow.<br>**Grass tile variants** (`generateTiles.ts`+`tileMap.ts`): added GRASS_VAR_A/B/C (TILE_COUNT 104→107), each with distinct blade-stroke seeds and cluster positions, wired into `TILE_VARIANT_POOL` 5-entry pool (base ×2 + 3 variants); wildflowers redistributed across variants instead of baked at fixed positions in every cell. `drawKenneyFallbacks` now stamps a magenta diagonal stripe in the top-left corner of every unmapped tile so a sprite-load failure visibly signals "broken sprite" instead of becoming a uniform green block.<br>**Phaser lifecycle + dev capture** (`PhaserGame.tsx`+`config.ts`): `import.meta.hot.dispose` cleanup destroys the previous game instance before HMR replaces the module (fixes 12× banner accumulation); `preserveDrawingBuffer: import.meta.env.DEV` so headless screenshots can capture the WebGL canvas during dev — zero perf impact in prod.<br>**Volume defaults** (`main.tsx`): `applyStoredOptions()` now runs at app boot instead of only on InGameOverlay mount, so main-menu audio + first-time options screen reflect the 70% defaults instead of falling through to mute.<br>**Save format hardening** (`saveLoad.ts` + new `saveLoad.test.ts`): centralized `normalizeSaveData()` fills 18 post-v1 optional fields with safe defaults at load time. Old v1 saves no longer crash on `.length`/`.get`/etc.; `loadGame` reads unguarded after normalization. +4 tests covering minimal v1 save, value preservation, nested time/weather defaults, nested worldState defaults.<br>**Dead code**: deleted unused `addAmbientVignette` from `pseudo3d.ts` (the call was already removed earlier in the session).<br>**Verified**: typecheck clean, **341/341 tests pass**, `npm run build` clean (~1.7s), production bundle confirmed free of `"Cheats (Playtest)"` and `"new Function"` strings. Skipped from this pass: A4 architecture sprite mappings (3 prior reverts in repo history — needs interactive pixel-verification, not delegation), B5/B10/B14/B18 (non-bugs, repro-needed, design choices, or profiling-required). |
| _(story drop, 3 parallel agents + spine in main)_ | **Story drop**: tied the world together with 4 things shipped in one session.<br>**1. Story spine** (mine, `ca7b160` + `0647d2f`): four-fold-seal canon in `lore/world.md` + `lore/timeline.md`. Each main-boss kill auto-unlocks a codex fragment (`BOSS_CODEX` in `combatStore.ts`); read in order they assemble the truth that the bosses ARE the king, split. New `brenna-revelation.json` cutscene fires when 2+ main bosses are dead — Brenna names her great-aunt Yorath (one of the Sealing Order) and tells the player what they have actually been doing. Player choice: continue / stop / think (sets `hc_revelation_*`).<br>**2. Companions** (agent-companions, `c8dc545` → `0b3ce13`): 3 recruitable companions with 3-beat arcs — Lyra Ashen (guild deserter at Mossbarrow Cairn, rank-aware encounter), Halvor Trent (Sundering scholar in Greenhollow), Quill (former Hollow King cultist at the inn, only spawns post-Hollow-King). 9 new dialogue files. `DialogueScene.readFlag` now accepts both `'1'` and `'true'` truthy forms.<br>**3. NPC personal quests** (agent-npc-quests, `fbd1a7e` → `9ca3ffe`): 6 quests, one per anchor NPC, each hits 2+ DESIGN.md §9.5 emotional notes — *The Deserter's Letter* (Brenna, sad/hard-decision; lie or truth about Hesta), *The Cloak in the Bramble* (Orric, heartbreak/sad), *The Empty Room* (Tomas, sad/puzzle/hard-decision; 3 recipient paths for the dead guest's journal), *The Last Ledger* (Vira, hard-decision/heartbreak; collect 40g vs pay it yourself), *The Forge He Couldn't Light* (Kael, heartbreak/puzzle; gather + lay sword on Roa's grave), *The Last Seal* (Veyrin, hard-decision/heartbreak/sad; reinforce or mercy). New NPCs Hesta (Orric's cabin) + Halden (Duskmere). Pattern: extending each greeting JSON with offer + turn-in branches, plus 2 standalone dialogue files for the new NPCs.<br>**4. Environmental lore** (agent-lore, `c1fae72` → `fc6df1c`): 12 gravestones (Greenhollow, Mossbarrow, Mossbarrow Depths F1, Drowned Sanctum F1), 6 journal pages by **Aelia Ren** (Sundering-era cartographer-sealer; 6-page arc from hopeful chartwork to "what we made was not a wall, it was a door we cannot reopen"; pages scattered across Mossbarrow Depths F1-3, Drowned Sanctum F1, Ashen Tower F1), 4 broken signs/inscriptions ("BUILT FOR THE LIVING / KEPT FOR THE DEAD / —order of the cairn", etc). Three new `BaseWorldScene` helpers: `spawnGravestone`, `spawnJournalPage`, `spawnBrokenSign`. All 22 entries unlock via `loreStore.discover()`; one-time-read via `worldStateStore.pickedObjects`.<br>**Verified**: typecheck clean, 337/337 tests pass, `npm run build` clean. Manual conflict resolution in 5 files (dialogues.ts, npcs.json, InteriorScene.ts, MossbarrowScene.ts, DialogueScene.tsx — all additive "kept both" merges). |
| _(prior drop, parallel-agent run)_ | **Four-feature simultaneous drop** orchestrated across 3 background agents (worktree-isolated) + main session. Pushed as 5 commits + 3 merge commits. Net: ~2700 lines added across combat, world content, items, and dialogue.<br>1. **Multi-enemy fights** (main, `0c367bb`) — `extraEnemies[]` on CombatState, AoE skill iteration, auto-target on attack, group-encounter spawn. Two early packs seeded.<br>2. **Reactive dialogue + persist memoryStore + Mira arc** (agent-dialogue, merged via `ffb2771` + `6d66337`) — 5 new declarative requirement predicates, 6 NPC reactive lines, 3-beat Mira arc with companion recruit. +17 tests.<br>3. **5 secret zone bosses** (agent-bosses, merged via `fb0caaa`) — 5 hidden-trigger bosses with procedural sprites + 5 unique drops, persisted via worldStateStore.pickedObjects.<br>4. **8 weapons + perk system** (agent-weapons, merged via `bfea348` + `e0022b8`) — discriminated-union `WeaponPerk` (on_hit_status, on_hit_heal, crit_range/mult, damage_type, damage_bonus_vs_weakness, flat_damage). Manual conflict resolution in combat.ts attack branch to compose perks with multi-enemy auto-targeting. +3 tests.<br>**Verified**: typecheck clean, **324/324 tests pass** (was 305), `npm run build` green. |
